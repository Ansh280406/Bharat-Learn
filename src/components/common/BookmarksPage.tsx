...
import { BookmarkX, BookOpen, Sparkles, ChevronRight } from 'lucide-react';
...
const SUBJECT_META: Record<string, { subject: string; emoji: string }> = {
  heart: { subject: 'Biology', emoji: '❤️' },
  water_cycle: { subject: 'Geography', emoji: '💧' },
  math: { subject: 'Mathematics', emoji: '📐' },
  math_3d: { subject: 'Mathematics', emoji: '🧠' },
  history: { subject: 'History', emoji: '⚔️' },
  physics: { subject: 'Physics', emoji: '🌈' },
  chemistry: { subject: 'Chemistry', emoji: '🧪' },
  unknown: { subject: 'AI Scan', emoji: '📚' },
};
    ...
    return (
      <div className="glass-card" style={{ padding: '60px 24px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
          Bookmark lessons from the Library or AR Scanner to find them here quickly.
        </p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingBottom: '24px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {bookmarks.map((bm, index) => {
          const meta = SUBJECT_META[bm.pageType] || SUBJECT_META.unknown;
          const color = SUBJECT_COLORS[meta.subject] || '#6366f1';
          return (
            <div
              key={bm.pageType + index}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
                  <button
                    onClick={() => onLaunchAR(bm.pageType as PageType)}
                  ...
                  <button
                    onClick={() => toggleBookmark(bm.pageType, bm.title)}
      ...
      <div className="glass-card-sm" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <p style={{ fontSize: '12px', color: 'var(--text-primary)', opacity: 0.85, lineHeight: '1.5' }}>
          Tip: Bookmark any lesson from the <strong style={{ color: 'var(--text-primary)' }}>AR Scanner</strong> by tapping the bookmark icon in the top right.
        </p>
      </div>
    </div>
  );
